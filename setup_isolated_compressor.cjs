...
console.log("1. Restoring src/utils/pdfEngine.ts to clean git HEAD...");
execSync("git checkout HEAD -- src/utils/pdfEngine.ts", { stdio: "inherit" });

console.log("2. Creating dedicated, isolated module src/utils/exactCompressor.ts...");
const exactCompressorCode = `import { PDFDocument } from 'pdf-lib';
...
export async function compressPDF(
  ...
  for (let pageNum = 1; pageNum <= totalPages; pageNum++) {
    ...
    // Step 1: Render safely with PDF.js at a stable scale (>= 0.65) to eliminate WASM overflows
    ...
    if (pdfCtx) {
      ...
      // Step 2: Native 2D Canvas hardware downscaling (runs in native C++, completely outside WASM)
...
fs.writeFileSync("src/utils/exactCompressor.ts", exactCompressorCode);
console.log("✓ src/utils/exactCompressor.ts created successfully.");

console.log("3. Updating src/components/Compressor.tsx to use exactCompressor...");
let compCode = fs.readFileSync("src/components/Compressor.tsx", "utf8");
compCode = compCode.replace(
  /from\s+['"]\.\.\/utils\/pdfEngine['"]/,
  "from '../utils/exactCompressor'"
);
fs.writeFileSync("src/components/Compressor.tsx", compCode);
console.log("✓ src/components/Compressor.tsx re-routed.");
