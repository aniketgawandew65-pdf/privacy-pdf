...
export async function extractPage(
  ...
  if (!meaningful) {
    if (preservation.artworkItems.size)
      throw Error(
        `Page ${page.pageNumber} has no recoverable editable text. Its font mappings need local text recognition, which this tool does not support.`,
      );
    throw Error(
      `Page ${page.pageNumber} has no extractable text. Scanned/image-only pages require local OCR, which is not included in this digital-PDF tool. No screenshot-only DOCX was created.`,
    );
  }
